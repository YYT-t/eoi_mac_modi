...
class Agent(object):
	def __init__(self,sess,observation_space,n_actions,state_space,n_ant,alpha):
		...
		self.q_nets = build_q_net(self.observation_space,self.n_actions)
		for i in range(self.n_ant):	
			self.critics.append(build_critic(self.observation_space,self.n_actions))
		self.mixer = build_mixer(self.n_ant,self.state_space)
		self.Q_tot = build_Q_tot(self.observation_space,self.n_actions,self.state_space,self.n_ant,self.q_nets,self.mixer)
		...
		self.Q_tot_tar = build_Q_tot(self.observation_space,self.n_actions,self.state_space,self.n_ant,self.q_nets_tar,self.mixer_tar)
		...
		loss_i = tf.reduce_mean([tf.nn.softmax_cross_entropy_with_logits(logits=self.q_nets(self.Q_tot.inputs[i]) - 9e15*(1 - self.action_masks[i]),labels=tf.nn.softmax(self.critics[i](self.Q_tot.inputs[i]) - 9e15*(1 - self.action_masks[i]))) for i in range(self.n_ant)])

		self.optimize.append(tf.train.AdamOptimizer(0.0005).minimize(loss_q + self.alpha*loss_i, var_list = self.q_nets.trainable_weights))
		...
		for i in range(self.n_ant):
			...
			self.opt_critic.append(tf.train.AdamOptimizer(0.0005).minimize(tf.reduce_mean((self.label_critic[i] - self.critics[i].outputs[0])**2), var_list = self.critics[i].trainable_weights))
